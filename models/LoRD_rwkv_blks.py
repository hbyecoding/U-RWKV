...
class GatedSpatialConvolution2D(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # First convolution block: 3x3 convolution
        self.conv2d_3x3 = nn.Sequential(
            nn.BatchNorm2d(in_channels),
            nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),
            nn.ReLU()
        )
        # Second convolution block: 1x1 convolution
        self.conv2d_1x1 = nn.Sequential(
            nn.BatchNorm2d(in_channels),
            nn.Conv2d(in_channels, out_channels, kernel_size=1),
            nn.ReLU()
        )
        # Final convolution block for feature fusion
        self.conv2d_fuse = nn.Sequential(
    ...
    def forward(self, z):
        ...
        conv2d_3x3 = self.conv2d_3x3(z)
        # Apply 1x1 convolution
        conv2d_1x1 = self.conv2d_1x1(z)
        # Gating mechanism: element-wise multiplication
        gated = conv2d_3x3 * conv2d_1x1
        # Fuse features
        fused = self.conv2d_fuse(gated)
        # Residual connection
        return z + fused

class BinaryOrientatedRWKV2D(nn.Module):
    def __init__(self, n_embd, n_layer, shift_mode='q_shift',
                 channel_gamma=1/4, shift_pixel=1, hidden_rate=4, init_mode='fancy',
                 drop_path=0., k_norm=True):
        super().__init__()
        # RWKV layer for forward direction
        ...
        # RWKV layer for reverse direction
    ...
    def forward(self, z):
        # Flatten input into two sequences
        B, C, H, W = z.shape
        z_f = z.flatten(2).transpose(1, 2)  # Forward direction
        z_r = z.transpose(2, 3).flatten(2).transpose(1, 2)  # Reverse direction .flip(2)

        # Process each sequence with RWKV
...
class BinaryRWKVLayer2D(nn.Module):
    def __init__(self, in_channels, out_channels, n_embd, n_layer, proj_view_shape, proj_axes, shift_mode='q_shift',
                 channel_gamma=1/4, shift_pixel=1, hidden_rate=4, init_mode='fancy',
                 drop_path=0., k_norm=True):
        super().__init__()
        # Gated Spatial Convolution (GSU)
        self.gsu = GatedSpatialConvolution2D(in_channels, out_channels)
        # Tri-orientated RWKV (ToM)
        self.tom = BinaryOrientatedRWKV2D(
            n_embd=n_embd,
            n_layer=n_layer,
            shift_mode=shift_mode,
            channel_gamma=channel_gamma,
        ...
        self.proj_view_shape = proj_view_shape
    ...
    def proj_feat(self, x):
        ...
        Returns:
            torch.Tensor: Reshaped and permuted tensor.
        """
    ...
    def forward(self, z):
        ...
        z_proj = self.proj_feat(z)

        # GSU module
        gsu_output = self.gsu(z_proj)

        # ToM module
        tom_output = self.tom(z_proj)

        # Binary-like fusion: sum of GSU and ToM outputs
        fused = gsu_output + tom_output

        # Reshape back to the original shape
        fused = fused.permute(self.proj_axes).contiguous()
        fused = fused.view(z.size(0), z.size(1), z.size(2), z.size(3))
...
class LoRD_128_192_384(nn.Module):
    def __init__(self, input_channel=3, num_classes=1,
        ...
        self.rwkv_bottleneck = VRWKV_Bottleneck(n_embd=dims[4],n_layer=12,layer_id=0,
                                     ...
                                     init_mode='fancy',
                                     k_norm=True)
        self.bottom = BinaryOrientatedRWKV2D(n_embd=dims[4],
                       n_layer=12, 
                       shift_mode='q_shift',
                       channel_gamma=1/4,
                       shift_pixel=1,
