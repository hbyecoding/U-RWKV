...
class LORDD(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_Dual:
            ...
            return out1     
def LORDD_for_FLOPs(input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], use_Dual=False, use_gs=False, use_WT = False):
    return LORDD(input_channel=input_channel, num_classes=num_classes, dims=dims,depths=depths,use_Dual=False, use_gs=False, use_WT = False)

def LORDD_s_for_FLOPs(input_channel=3, num_classes=1, dims=[8, 16, 32, 64, 128], depths=[1, 1, 1, 1, 1], use_Dual=False, use_gs=False, use_WT = False):
    return LORDD(input_channel=input_channel, num_classes=num_classes, dims=dims,depths=depths, use_Dual=False, use_gs=False, use_WT = False)

def LORDD_l_for_FLOPs(input_channel=3, num_classes=1, dims=[32, 64, 128, 256, 512], depths=[1, 1, 1, 6, 3], use_Dual=False, use_gs=False, use_WT = False):
    return LORDD(input_channel=input_channel, num_classes=num_classes, dims=dims,depths=depths, use_Dual=False, use_gs=False, use_WT = False)
...
#     x = torch.rand((1, 3, 256, 256)).cuda()
    ...
    model = LORDD_l_for_FLOPs(use_Dual=True,use_gs=True)
    model.cuda()
