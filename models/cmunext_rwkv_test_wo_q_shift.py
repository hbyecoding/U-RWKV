...
# def q_shift(input,  shift_pixel=1, gamma=0.25, patch_resolution=None, add_residual=True,N_layers = 8):
#     """
...
#     # conv_by = Conv(C, C//N_layers, 1, 1).cuda()
#     # conv_N = Conv(C//N_layers, C, 1, 1).cuda()    
#     # 计算每个方向的通道数
#     gamma_C = int(C * gamma)

#     # x_by_n_layer = conv_by(input)
#     # output = torch.zeros_like(x_by_n_layer)
#     x_by_n_layer = input
#     # 右移
...
#     """
#     Q-Shift 操作的 4D 版本，输入形状为 (B, C, H, W)。
#     zagq_shift_wo_res name 只能放在 model那儿了
#     Args:
#         input: 输入张量，形状为 (B, C, H, W)。
#         shift_pixel: 每个方向的移动距离。
#         gamma: 每个方向的通道数比例。
...
#     # 右移
#     output[:, 0:gamma_C:4, :, shift_pixel:W] = input[:, 0:gamma_C:4, :, 0:W-shift_pixel]
#     # 左移
#     output[:, 1:gamma_C:4, :, 0:W-shift_pixel] = input[:, 1:gamma_C:4, :, shift_pixel:W]
#     # 下移
#     output[:, 2:gamma_C:4, shift_pixel:H, :] = input[:, 2:gamma_C:4, 0:H-shift_pixel, :]
#     # 上移
#     output[:, 3:gamma_C:4, 0:H-shift_pixel, :] = input[:, 3:gamma_C:4, shift_pixel:H, :]
#     # 保留剩余通道
#     output[:, gamma_C:, ...] = input[:, gamma_C:, ...]
...
def q_shift(input, shift_pixel=1, gamma=0.25, patch_resolution=None, add_residual=True, wo_shift=True):
    """
    Q-Shift 操作的 4D 版本，输入形状为 (B, C, H, W)。
...
class CMUNeXt_rwkv_1_6_256_512_768(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[16, 64, 256,512, 768], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7], args_vit=None):
        super().__init__()
        self.Maxpool = nn.MaxPool2d(2, 2)
        self.stem = conv_block(input_channel, dims[0])
        self.encoder1 = CMUNeXtBlock(dims[0], dims[0], depths[0], kernels[0])
...
class CMUNeXt_rwkv_1_6_256_512_768_qzagshift(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[16, 64, 256,512, 768], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7], args_vit=None):
        super().__init__()
        self.Maxpool = nn.MaxPool2d(2, 2)
        self.stem = conv_block(input_channel, dims[0])
...
if __name__ == "__main__":
    # 假设 model 是动态创建的
    model = CMUNeXt_rwkv_1_3_128_256_768()  # 这里可以替换为其他模型

    # 获取模型的名称
    model_name = model.__class__.__name__ +"_wo_q_shift"
