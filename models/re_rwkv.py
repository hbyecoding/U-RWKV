...
# wkv_cuda = load(name="wkv", sources=["Tan9/src/network/conv_based/model/cuda/wkv_op.cpp", "Tan9/src/network/conv_based/model/cuda/wkv_cuda.cu"],
#                 verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}'])
wkv_cuda = load(name="wkv", sources=["Tan9/src/network/conv_based/transformer_learning/cuda/wkv_op.cpp", "Tan9/src/network/conv_based/transformer_learning/cuda/wkv_cuda.cu"],
                verbose=True, extra_cuda_cflags=['-res-usage', '--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}'])
...
if __name__ == "__main__":
    ...
    x=torch.zeros((1, 48, 128, 128)).cuda() #.type(torch.FloatTensor)
    # model = Restore_RWKV() 
    model = Block(n_embd=48,n_layer=12, layer_id=0).cuda() 
    # model.cuda() 
