...
# for x=torch.zeros((1, 3, 256, 256)).type(torch.FloatTensor).cuda()
# def apply_rotary_pos_emb(x, sin, cos, scale=1):
#     sin, cos = map(lambda t: duplicate_interleave(t * scale), (sin, cos))
#     # Ensure cos and sin have the same shape as x
#     import pdb;pdb.set_trace()
#     cos = cos[:, :, :x.shape[-1]]  # (batch_size, sequence_length, head_size)
#     sin = sin[:, :, :x.shape[-1]]  # (batch_size, sequence_length, head_size)
...
# for x=torch.zeros((1, 3, 256, 256)).type(torch.FloatTensor).cuda()    
    ...
    #     # Ensure cos and sin have the same shape as x
    #     cos = cos.unsqueeze(0).expand(x.shape[0], -1, -1)  # (batch_size, sequence_length, head_size)
    #     sin = sin.unsqueeze(0).expand(x.shape[0], -1, -1)  # (batch_size, sequence_length, head_size)
...
class SimpleRetention(nn.Module):
    ...
    #     Q = self.xpos(Q)  # Apply rotary position embedding
    #     K = self.xpos(K, downscale=True)  # Apply rotary position embedding
