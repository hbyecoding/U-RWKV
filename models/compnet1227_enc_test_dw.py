...
class EncoderBlock_DW(nn.Module):
    def __init__(self, in_channels, out_channels, depth=1, k=3):
        super(EncoderBlock_DW, self).__init__()
        self.conv = nn.Sequential(
        ...
        self.out = ConvBlock(in_channels, out_channels)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
...
class LoRA_DW__5(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[24,48,96,192,384], depths=[1, 1, 1, 3, 1], kernel_sizes=[3,3,7,7,7], expansion_factors=[4] * 5):
        ...
        # self.e5 = LoRDBlockEnc(dims[3], dims[4])
        self.n_emb = dims[-1]
        self.e1 = EncoderBlock_DW(input_channel, dims[0],depth=depths[0],k=kernel_sizes[0])
        self.e2 = EncoderBlock_DW(dims[0], dims[1],depth=depths[1],k=kernel_sizes[1])
        self.e3 = EncoderBlock_DW(dims[1], dims[2],depth=depths[2],k=kernel_sizes[2])
        self.e4 = EncoderBlock_DW(dims[2], dims[3],depth=depths[3],k=kernel_sizes[3])
        self.e5 = EncoderBlock_DW(dims[3], dims[4],depth=depths[4],k=kernel_sizes[4])
        """ Decoder: Segmentation """
        self.s1 = DecoderBlock(dims[4], dims[3])
        self.s2 = DecoderBlock(dims[3], dims[2])
        self.s3 = DecoderBlock(dims[2], dims[1])
...
class LoRA_dw_4_5(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[24,48,96,192,384], depths=[1, 1, 1, 3, 1], kernel_sizes=[[3, 3]] * 5, expansion_factors=[4] * 5):
        super(LoRA_dw_4_5, self).__init__()
        self.nemb = dims[-1]
        ...
        self.a4 = DecoderBlock(dims[1], dims[0])
        self.a5 = DecoderBlock(dims[0], 16)
        self.Brwkv_by_2 = BinaryOrientatedRWKV2D(n_embd=self.nemb_by_2, 
                                            n_layer=12, 
                                            shift_mode='q_shift',
                                            channel_gamma=1/4,
                                            shift_pixel=1,
        ...
        self.Brwkv_5 = BinaryOrientatedRWKV2D(n_embd=self.nemb, 
                                            n_layer=12, 
                                            shift_mode='q_shift',
                                            channel_gamma=1/4,
                                            shift_pixel=1,
        ...
        self.Brwkv_mul_2 = BinaryOrientatedRWKV2D(n_embd=self.n_emb*2, 
                                            n_layer=12, 
                                            shift_mode='q_shift',
                                            channel_gamma=1/4,
                                            shift_pixel=1,
        ...
        """fusion"""    
        # 设置到 x4身上
        self.withDW_by_2 = ProProcessWithDWConv(ch_in=dims[-2] // 2, ch_out=dims[-2],depth=1)
        self.withDW_by_4 = ProProcessWithDWConv(ch_in=dims[-2] // 4, ch_out=dims[-2],depth=2)
        
        self.withDW_4_2_5 = ProProcessWithDWConv(ch_in=dims[-1] // 2, ch_out=dims[-1],depth=1)
        ...
        self.bot2_5_conv = nn.Sequential(
    ...
    def forward(self, x):
        ...
        x4_by_4 = self.withDW_by_4(x2)
        x4_by_2 = self.withDW_by_2(x3)
        
        # +Nconv
        x4_pre = torch.cat([x4_by_4,x4_by_2], dim=1)
        x4_pre = self.bot2_conv(x4_pre)
        
        x4_rwkv_cat_conv = self.Brwkv_by_2(x4_pre) #c = nemb /2, h = * 2,w  = *2
        p4 = self.Brwkv_by_2(p4)
        

        x5, p5 = self.e5(p4)
        x5_by_2 = self.withDW_4_2_5(x4_rwkv_cat_conv)
        
        x5_pre = torch.cat([x5_by_2,x5],dim=1)
        x5_pre = self.bot2_5_conv(x5_pre)
        x5 = self.Brwkv_5(x5_pre) # x5_rwkv_cat_conv
        p5 = self.Brwkv_5(p5)
        # print(x4.shape, p4.shape)
        # print(x5.shape, p5.shape)
        """"fusion"""
...
class LoRA_dw_rwkv_first_4_5(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[24,48,96,192,384], depths=[1, 1, 1, 3, 1], kernel_sizes=[[3, 3]] * 5, expansion_factors=[4] * 5):
        super().__init__()
        self.nemb = dims[-1]
        self.nemb_by_2 = dims[-2]
        ...
        self.Brwkv_mul_2 = BinaryOrientatedRWKV2D(n_embd=self.nemb*2, 
                                            n_layer=12, 
                                            shift_mode='q_shift',
                                            channel_gamma=1/4,
                                            shift_pixel=1,
        ...
        self.withDW_384_384_d1 = ProProcessWithDWConv(ch_in=dims[-1], ch_out=dims[-1],depth=1)
    ...
    def forward(self, x):
        ...
        x4_rwkv_cat = self.Brwkv_5(x4_pre) #c = nemb  , h = * 2,w  = *2  384 32 32
        
        x4_dwconv_rwkv_cat = self.withDW_384_384_d1(x4_rwkv_cat)
        # self.bot2_conv(x4_rwkv_cat) 注意维度不能使用 
        
        # p4 = self.Brwkv_by_2(p4)
        

        x5, p5 = self.e5(p4)
        x5_by_2 = x4_dwconv_rwkv_cat #self.withDW_4_2_5(x4_rwkv_cat_conv)
        
        x5_cat = torch.cat([x5_by_2,x5],dim=1)
        
        x5_rwkv_cat = self.Brwkv_mul_2(x5_cat)
        
        x5_pre = self.bot2_5_conv(x5_rwkv_cat)
        x5 = self.Brwkv_5(x5_pre) # x5_rwkv_cat_conv
        # p5 = self.Brwkv_5(p5)
        # print(x4.shape, p4.shape)
        # print(x5.shape, p5.shape)
