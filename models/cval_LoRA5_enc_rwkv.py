...
wkv_cuda = load(name="wkv", sources=[os.path.join('/','data','hongboye','projects','Tan9','src','network', 'conv_based','transformer_learning', 'cuda', 'wkv_op.cpp'), os.path.join('/','data','hongboye','projects','Tan9','src','network', 'conv_based', 'transformer_learning', 'cuda', 'wkv_cuda.cu')],
                verbose=True, extra_cuda_cflags=['-res-usage','--maxrregcount 60', '--use_fast_math', '-O3', '-Xptxas -O3', f'-DTmax={T_MAX}']) #'--maxrregcount 60', 
...
def v_enc_256_fffse_dec_resi2_rwkv_withbirwkv(dims=[16,32,128,160,256],in_channels=3, num_classes=1): #_withinpool
    class ConvBnoptinalAct(nn.Module):
        def __init__(self, in_channels, out_channels,kernel_size,padding, with_act=None):
            super(ConvBnoptinalAct, self).__init__()
            self.conv = nn.Sequential(
    ...
    class VEncSym(nn.Module):
        def __init__(self, input_channel=in_channels, num_classes=num_classes, dims=dims, depths=[1, 1, 1, 3, 1], kernel_sizes=[[3, 3]] * 5, expansion_factors=[4] * 5):
            super(VEncSym, self).__init__()
            self.n_emb = dims[-1]
            """ Shared Encoder """
            self.stem = nn.Sequential(
...
def v_enc_256_fffse_dec_resi2_rwkv_with2x4(dims=[16,32,128,160,256],in_channels=3, num_classes=1): #_withinpool
    class ConvBnoptinalAct(nn.Module):
        def __init__(self, in_channels, out_channels,kernel_size,padding, with_act=None):
            super(ConvBnoptinalAct, self).__init__()
            self.conv = nn.Sequential(
...
def v_enc_256_fffse_dec_fusion_with2x4(dims=[16, 32, 128, 160, 256],in_channels=3, num_classes=1): #_withinpool
    class ConvBnoptinalAct(nn.Module):
        def __init__(self, in_channels, out_channels,kernel_size,padding, with_act=None):
            super(ConvBnoptinalAct, self).__init__()
            self.conv = nn.Sequential(
...
def v_enc_256_fffse_dec_fusion_rwkv_with2x4(dims=[16, 32, 128, 160, 256], in_channels=3, num_classes=1): #_withinpool
    class ConvBnoptinalAct(nn.Module):
        def __init__(self, in_channels, out_channels,kernel_size,padding, with_act=None):
            super(ConvBnoptinalAct, self).__init__()
            self.conv = nn.Sequential(
