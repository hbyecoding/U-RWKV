...
class DWConvWithGate(nn.Module):
    def __init__(self, in_channels, kernel_sizes=[3, 3], expansion_factor=4):
        ...
        for kernel_size in kernel_sizes:
            ...
            branch = nn.Sequential(
                ...
                LayerNorm(in_channels * expansion_factor, data_format="channels_first"),  
                nn.GELU(),
        ...
        self.normx1 = LayerNorm(in_channels, data_format='channels_first')  # Normalization
        self.normx2 = LayerNorm(in_channels*2, data_format='channels_first')  # Normalization
    ...
    def forward(self, x):
        ...
        gate_output = self.normx1(self.out_conv(self.normx2(concat_output)))
    ...
    def __init__(self, ch_in, ch_out, depth=1, kernel_sizes=[3, 5], expansion_factor=4):
        ...
        self.block = nn.Sequential(
            *[Residual(DWConvWithGate(ch_in, kernel_sizes, expansion_factor)) for _ in range(depth)],
            ...
            LayerNorm(ch_out, data_format="channels_first"),  
            nn.GELU()
        )
...
class FusionConv(nn.Module):
    def __init__(self, ch_in, ch_out):
        ...
        self.conv = nn.Sequential(
            ...
            nn.GELU(),
            LayerNorm(ch_in, data_format="channels_first"),  
            nn.Conv2d(ch_in, ch_out * 4, kernel_size=(1, 1)),
            nn.GELU(),
            LayerNorm(ch_out * 4, data_format="channels_first"),  
...
class LoG(nn.Module):
    def __init__(self, input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernel_sizes=[[3,5],[5,7],[5,7],[3,7],[3, 5]], expansion_factors=[2]*5):
