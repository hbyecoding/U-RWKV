...
if __name__ == "__main__":
    input = torch.randn(1, 64, 8, 8) #输入 B C H W torch.randn(1, 64, 128, 128)
    
# 这里我需要一个真实的图片。 看看 能 怎么样？

    block = CAA(64)
    output = block(input)
    log.add('/data/hongboye/projects/model/plugs/model_of_seg.log')
    log.info("cvpr2024caa_NCHW NCHW_2024_11_22")
    log.info(block.named_modules)
    print(input.size())
    print(output.size())