...
class SimpleViT(nn.Module):
    ...
    def forward(self, series):
        *_, n, dtype = *series.shape, series.dtype
        ...
        xre = rearrange(x, 'b ... d -> b (...) d') + pe

        xtr = self.transformer(xre)
        xmean = xtr.mean(dim = 1)

        xout = self.to_latent(xmean)
        return self.linear_head(xout)
...
if __name__ == '__main__':
    ...
    time_series = torch.randn(4, 3, 256) # BS, L, C
